...
router.post('/', async (req, res) => {
  try {
    const { barcode, name, price, stock, expiration } = req.body;

    let existing = await Product.findOne({ barcode });
    if (existing) return res.status(400).json({ error: 'Código de barras ya registrado' });

    const newProduct = new Product({ barcode, name, price, stock: stock || 0, expiration });
...
router.put('/:id', async (req, res) => {
  try {
    ...
    const updated = await Product.findByIdAndUpdate(
      req.params.id,
      { barcode, name, price, stock, expiration },
      { new: true },
    );
